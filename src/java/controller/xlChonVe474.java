...
//        out.close();
        ...
        switch (action) {
            ...
            case "chonGhe" -> {
                String maGhe = request.getParameter("ghe");
                session.setAttribute("maGhe", maGhe);
                getServletContext().getRequestDispatcher("/gdChonVe474.jsp").forward(request, response);
            }
            case "luu" -> {
                int maLichChieu = Integer.parseInt((String) session.getAttribute("maLichChieu"));
                int maGhe = Integer.parseInt((String) session.getAttribute("maGhe"));
                
                Ve474 ve = new Ve474(
                        (new LichChieuDao474()).getLichChieu(maLichChieu),
                        (new GheDao474()).getGhe(maGhe)
                );
